...
type ApiValue = null | boolean | number | string | ApiValue[] | { [key: string]: ApiValue };
...
function objects(payload: ApiValue): ApiObject[] {
...
function display(value: ApiValue | undefined): string {
...
export function ConnectedResource({ endpoint, fields, itemPathPrefix, itemPathKey = "id", emptyMessage, ariaLabel }: ConnectedResourceProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    void fetch(endpoint, { credentials: "same-origin", cache: "no-store", headers: { accept: "application/json" }, signal: controller.signal })
      .then(async (response) => {
        ...
        const items = objects(await response.json() as ApiValue);
  ...
  if (state.kind === "loading") return <p role="status" aria-live="polite">Chargement depuis l’API locale…</p>;
  ...
  if (state.kind === "empty") return <p role="status">{emptyMessage}</p>;
  return <div className="table-scroll"><table aria-label={ariaLabel}><thead><tr>{fields.map((field) => <th key={field.key}>{field.label}</th>)}{itemPathPrefix ? <th>Action</th> : null}</tr></thead><tbody>{state.items.map((item, index) => {
    const key = display(item.id ?? item.leadCode ?? index);
    const itemIdentifier = apiString(item, itemPathKey);
    return <tr key={key}>{fields.map((field) => <td key={field.key}>{display(item[field.key])}</td>)}{itemPathPrefix && itemIdentifier ? <td><a href={`${itemPathPrefix}/${encodeURIComponent(itemIdentifier)}`}>Ouvrir</a></td> : itemPathPrefix ? <td>—</td> : null}</tr>;
  })}</tbody></table></div>;
}
