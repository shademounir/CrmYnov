...
export default function RolesPage(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    void permissionRequest<Catalogue>(`catalogue${campus ? `?campus=${encodeURIComponent(campus)}` : ""}`).then(async (data) => {
    ...
    }).catch((failure: unknown) => { if (current) { setError(message(failure)); setBusy(false); } });
  ...
  async function operation(action: () => Promise<void>): Promise<void> {
    ...
    try { await action(); } catch (failure) { setError(message(failure)); setPreview(null); setConfirmed(false); } finally { setBusy(false); }
  }
  ...
  return <main><PageHeader eyebrow="Administration · sécurité" title="Rôles et permissions" description="Droits configurables, plafonds explicites et protections métier intangibles." />
    ...
    {busy ? <p role="status">Chargement / validation en cours…</p> : null}{error ? <p role="alert">{error}</p> : null}{success ? <p role="status">{success}</p> : null}
    <button type="button" disabled={busy} onClick={() => setRevision((value) => value + 1)}>Recharger depuis le serveur</button>
    {catalogue ? <section className="panel permission-toolbar"><label>Campus<select value={configuration?.campus ?? campus} disabled={busy} onChange={(event) => { setCampus(event.target.value); setSuccess(""); }}>{catalogue.global ? <option value="GLOBAL">Plafond global / rôles globaux</option> : null}{catalogue.campuses.map((item) => <option key={item.id} value={item.id}>{item.code}</option>)}</select></label><label>Rôle système<select value={role} disabled={busy} onChange={(event) => { setRole(event.target.value); setSuccess(""); }}>{catalogue.global ? <option value="*">Plafond de toutes les permissions</option> : null}{catalogue.roles.map((item) => <option key={item.role} value={item.role}>{item.label} — {item.users} utilisateurs{item.editable ? "" : " · lecture seule"}</option>)}</select></label><p>5 rôles système · suppression interdite · registre v{catalogue.catalogueVersion}</p></section> : null}
    {configuration && catalogue ? <>
      ...
      <section className="panel permission-toolbar"><label>Identifiant technique du lead (facultatif)<input value={leadId} maxLength={36} onChange={(event) => { setLeadId(event.target.value); setExplanation(null); }} /></label><button disabled={busy} type="button" onClick={() => void operation(async () => { const query = new URLSearchParams({ campus: configuration.campus }); if (leadId) query.set("leadId", leadId); setExplanation(await permissionRequest<Explanation>(`effective?${query}`)); })}>Expliquer mes droits dans ce contexte</button></section>
