...
import { SharedViewControls } from "./shared-views";
import { createPrivateViewReader } from "./shared-view-client";
...
export function SavedViews({ current, resetHref = "/leads" }: Readonly<{ current: Record<string, string>; resetHref?: string }>): React.JSX.Element {
  ...
  const privateViews = useMemo(() => createPrivateViewReader<View>(), []);
  const filters = useMemo(() => Object.fromEntries(Object.entries(current).filter(([key, value]) => key !== "page" && key !== "pageSize" && key !== "sharedViewId" && value)), [current]);
  useEffect(() => {
    let generation = 0;
    const load = (): void => {
      const current = ++generation;
      void privateViews.read().then((rows) => { if (current === generation) setViews(rows); }).catch(() => { if (current === generation) setViews([]); });
    };
    const changed = (): void => { privateViews.invalidate(); load(); };
    load(); window.addEventListener("saved-views-changed", changed);
    return (): void => { generation++; window.removeEventListener("saved-views-changed", changed); };
  }, [privateViews]);
  async function save(): Promise<void> {
    ...
    if (!response.ok) { setState("error"); return; } const created = await response.json() as View; setViews((items) => [created, ...items]); setName(""); setState("success"); window.dispatchEvent(new window.Event("saved-views-changed"));
  }
  ...
  return <><section className="saved-view-controls" aria-label="Vues enregistrées">
    <div className="saved-view-controls__row"><strong>Mes vues</strong><Link className="secondary-button" href={resetHref}>Réinitialiser les filtres</Link></div>
  ...
  </section><SharedViewControls current={filters} readPrivateViews={privateViews.read} /></>;
}
