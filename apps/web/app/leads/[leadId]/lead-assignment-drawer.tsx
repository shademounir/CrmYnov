...
export function LeadAssignmentDrawer({ leadId, leadCode, assigned, onCompleted }: Readonly<{ leadId: string; leadCode: string; assigned: boolean; onCompleted?: () => void }>): React.JSX.Element {
  ...
  return <><button ref={trigger} className="secondary-button" type="button" onClick={() => { setDirty(false); dialog.current?.showModal(); }}><UserSwitch size={18} aria-hidden="true" /> {assigned ? "Réaffecter" : "Affecter"}</button>
