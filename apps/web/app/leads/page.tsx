...
export default function LeadsPage(): React.JSX.Element {
  return <main className="leads-page">
    <PageHeader eyebrow="Base prospects" title="Tous les leads" description="Centralisez, qualifiez et affectez chaque opportunité." actions={<LeadCreationDrawer />} />
    <DashboardReturnLink />
    ...
    <section className="panel leads-work-panel">
      <Suspense fallback={null}><LeadSavedViews /></Suspense>
      <form className="lead-filter-form" action="/leads" method="get" aria-label="Recherche et filtres des leads">
        <div className="lead-toolbar"><label><span className="sr-only">Identité ou identifiant</span><input name="search" placeholder="Rechercher par nom, email, téléphone ou LD-…" /></label><label><span className="sr-only">Statut</span><select name="status"><option value="">Tous les statuts</option><option value="PROSPECT">Prospect</option><option value="CONTACTED">Contacté</option><option value="QUALIFIED">Qualifié</option><option value="ENROLLED">Inscrit</option><option value="CLOSED_LOST">Sans suite</option></select></label><label><span className="sr-only">Température</span><select name="temperature"><option value="">Toutes les températures</option><option value="UNEVALUATED">Non évalué</option><option value="COLD">Froid</option><option value="WARM">Tiède</option><option value="HOT">Chaud</option></select></label><button className="secondary-button" type="submit"><Funnel size={18} /> Appliquer</button></div>
...
function LeadSavedViews(): React.JSX.Element {
  const params = useSearchParams();
  return <SavedViews current={Object.fromEntries(params.entries())} />;
}
