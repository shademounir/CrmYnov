...
export function ImportReport({ jobId }: Readonly<{ jobId: string }>): React.JSX.Element {
  const [report, setReport] = useState<Report | null>(null); const [error, setError] = useState("");
  useEffect(() => { void fetch(`${API}/lead-import/reports/${encodeURIComponent(jobId)}`, { credentials: "include" })
    .then(async (response) => response.ok ? response.json() as Promise<Report> : Promise.reject(new Error("report_load_failed")))
    .then(setReport).catch(() => setError("Rapport indisponible ou accès refusé.")); }, [jobId]);
