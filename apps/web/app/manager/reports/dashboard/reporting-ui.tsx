...
export default function InteractiveReportingDashboard({ initialFilters, initialReport }: { initialFilters: Record<string, string>; initialReport?: ReportingReport }): React.JSX.Element {
  ...
  const query = useMemo(() => { const params = new URLSearchParams(initialFilters); if (!params.has("period")) params.set("period", "30d"); return params; }, [initialFilters]);
  useEffect(() => {
    try { const saved = localStorage.getItem(preferenceKey); if (saved) setPreferences(safePreferences(JSON.parse(saved) as Partial<Preferences>)); } catch { localStorage.removeItem(preferenceKey); }
  }, []);
  useEffect(() => {
    ...
    fetch(`/api/reports/${query.get("view") === "personal" ? "personal-dashboard" : "manager-dashboard"}?${query.toString()}`, { credentials: "same-origin", signal: controller.signal, headers: { accept: "application/json" } })
      .then(async (response) => { if (!response.ok) throw new Error(`reporting_${response.status}`); return response.json() as Promise<ReportingReport>; })
      .then((value) => { setReport(value); const count = "cards" in value ? Object.values(value.cards).reduce((sum, item) => sum + item, 0) : value.performance.advisers.length + value.contributions.contributors.length; setState(count > 0 ? "ready" : "empty"); })
  ...
  return <main data-density={preferences.compact ? "compact" : "comfortable"}>
    ...
    <fieldset><legend>Préférences locales non sensibles</legend>
      ...
      <a href={`/manager/reports/dashboard?period=${preferences.preferredPeriod}`}>Appliquer la période préférée</a>
    ...
    {state === "loading" && <section aria-live="polite" aria-busy="true"><h2>Chargement</h2><p>Calcul des indicateurs agrégés…</p></section>}
    {state === "error" && <section role="alert"><h2>Erreur de chargement</h2><p>Le rapport n’a pas pu être chargé. Réessayez sans modifier vos filtres.</p></section>}
    {state === "empty" && <section aria-live="polite"><h2>Aucun résultat</h2><p>Aucune donnée agrégée pour les filtres sélectionnés.</p></section>}
    {state === "ready" && report && ("cards" in report ? <DashboardContent report={report} showTables={preferences.showTables} query={query} operationalThreshold={preferences.operationalThreshold} /> : <PersonalDashboardContent report={report} showTables={preferences.showTables} />)}
  </main>;
}

function PersonalDashboardContent({ report, showTables }: { report: PersonalDashboardReport; showTables: boolean }): React.JSX.Element {
...
function ReportingFilters({ filters }: { filters: Record<string, string> }): React.JSX.Element {
...
function DashboardContent({ report, showTables, query, operationalThreshold }: { report: DashboardReport; showTables: boolean; query: URLSearchParams; operationalThreshold: number }): React.JSX.Element {
  ...
  return <>
    <section aria-label="Cartes KPI"><h2>Indicateurs clés</h2><ul>{report.drillDowns.map((item) => <li key={item.key}><a href={item.href}><strong>{labels[item.key] ?? item.key}</strong> : {item.count}</a></li>)}<li><strong>Alertes actives</strong> : {report.cards.activeAlerts}</li></ul></section>
    ...
    <p><a href={report.export.href} download="crm-manager-dashboard-v1.csv">Exporter les agrégats CSV</a></p>
    <p><small>Contrat {report.definitionVersion} — export {report.export.schemaVersion} — {report.timezone}</small></p>
  </>;
}

function AccessibleChart({ title, data, showTable }: { title: string; data: Datum[]; showTable: boolean }): React.JSX.Element {
  const max = Math.max(1, ...data.map((item) => item.count));
  return <figure><figcaption><h2>{title}</h2></figcaption>
    <div role="img" tabIndex={0} aria-label={`${title}. ${data.map((item) => `${item.value}: ${item.count}`).join(", ") || "aucune valeur"}`}>
...
function AccessibleTrend({ data, showTable }: { data: DashboardReport["trends"]; showTable: boolean }): React.JSX.Element {
...
function preserveFilters(href: string, query: URLSearchParams): string {
  const [path, current = ""] = href.split("?"); const params = new URLSearchParams(current);
  for (const [key, value] of query) if (!params.has(key) && !["period", "view"].includes(key)) params.set(key === "from" ? "createdFrom" : key === "to" ? "createdTo" : key === "adviserId" ? "assignedToId" : key, value);
  params.set("returnTo", `/manager/reports/dashboard?${query.toString()}`); return `${path}?${params.toString()}`;
}
