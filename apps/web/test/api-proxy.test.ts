...
test("proxy policy accepts only bounded internal origins and safe relative segments", () => {
  ...
  assert.equal(apiOrigin({ NODE_ENV: "development" }), "http://127.0.0.1:3001");
  assert.equal(apiOrigin({ NODE_ENV: "production" }), "http://api:3001");
...
test("proxy policy fails closed for external or traversal-shaped inputs", () => {
  for (const environment of [{ NODE_ENV: "test" }, { CRM_API_INTERNAL_URL: "https://api.example.test/path" }, { CRM_API_INTERNAL_URL: "file:///tmp/api" }]) {
