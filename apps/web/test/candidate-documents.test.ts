...
test("renders API-backed document metadata without content or PII", () => { const html = renderToStaticMarkup(CandidateDocumentDashboardPage()); for (const text of ["Suivi documentaire", "métadonnées autorisées", "aucun contenu documentaire", "Chargement depuis l’API locale"]) assert.ok(html.includes(text)); assert.equal(html.includes("@example"), false); });
