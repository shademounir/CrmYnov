...
test("combines identity, identifier, adviser and business filters", () => {
  ...
  const lead = service.registerLocalLead({ ...base, leadCode: "LD-SEARCH-001", status: "QUALIFIED", temperature: "HOT", temperatureLabel: "Chaud", qualificationVersion: 1 });
  ...
  const filtered = service.listLeads({ page: 1, pageSize: 25, search: "Camille", assignedToId: base.assignedToId,
    status: "qualified", temperature: "hot", source: "form", program: "Programme A", campaign: "Campagne A", campus: "Campus A", sortBy: "leadCode", sortDirection: "asc" }, principal, "corr-combined");
...
test("rejects invalid query values fail-closed", () => {
  ...
  assert.throws(() => service.listLeads({ page: 1, pageSize: 25, temperature: "BURNING" }, principal, "corr"), hasCode("lead_temperature_filter_invalid"));
