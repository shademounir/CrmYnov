...
test("reports explicit workload, reactivity, approval and source alerts without PII", () => {
  ...
  const reportingAt = new Date(scheduledAt.getTime() + 24 * 60 * 60 * 1_000);
  ...
  followUps.schedule(assigned.id, { dueAt: scheduledAt.toISOString(), reason: "Relance synthétique" }, adviser, "corr-follow");
  ...
  const report = service.read({ noInteractionHours: "1", capacityWarningPercent: "90", loadGap: "1", sourceRiskPercent: "30", minSourceVolume: "3" }, manager, "corr-report", reportingAt);
